...
aupr_ratio_matrix=matrix(nrow=10,ncol=length(win_len))
rownames(aupr_ratio_matrix)=path_list
colnames(aupr_ratio_matrix)=win_len
for (l in win_len){
...
print(j)
aupr_ratio_matrix[j,10*l]=aupr_ratio
print(paste("auprc_ratio:",aupr_ratio))
#print(median(aupr_list))
}
}
print(aupr_ratio_matrix)
write.csv(aupr_ratio_matrix,file="/public/home/yaomt/win_auprc/win50_auprc_matrix.csv",row.names=F)
